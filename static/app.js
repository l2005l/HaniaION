...
(() => {
  ...
  window.addEventListener("haniaion-native-gnss", event => {
    ...
    set("nativeCn0", cn0 && cn0 > 0 ? cn0.toFixed(1) : "—");
    set("nativeAccuracy", accuracy && accuracy > 0 ? `±${accuracy.toFixed(1)} m` : "—");
    ...
    const constellationText = Object.entries(constellations)
      ...
      .map(([name, n]) => `${name} ${n}`)
      .join(" · ");
    set("nativeConstellations", constellationText || "ממתין לזיהוי מערכות");

    const lat = finite(d.latitude);
    const lon = finite(d.longitude);
    const speed = finite(d.speedMps);
    const locationTime = finite(d.locationTimeMs) || finite(d.timestamp) || Date.now();

    spoofScore = Math.max(0, spoofScore - 4);
    let lastDistance = null;
    let impliedSpeed = null;

    if (previous && lat !== null && lon !== null && previous.lat !== null && previous.lon !== null) {
      const dt = (locationTime - previous.time) / 1000;
      if (dt > 0.25 && dt < 15) {
        lastDistance = distanceMeters(previous.lat, previous.lon, lat, lon);
        impliedSpeed = lastDistance / dt;

        if (accuracy !== null && accuracy <= 25 && used >= 4 &&
            lastDistance > Math.max(100, accuracy * 5) && impliedSpeed > 45) {
          spoofScore += 30;
        }
        if (speed !== null && impliedSpeed > Math.max(35, speed * 4 + 12)) {
          spoofScore += 20;
        }
        if (speed !== null && speed < 1.5 && lastDistance > 60 && dt < 5) {
          spoofScore += 25;
        }
        if (impliedSpeed > 120 && lastDistance > 150) {
          spoofScore += 30;
        }
      }
    }
    ...
    if (note) {
      let quality = "ממתין ל־Fix יציב";
      if (used >= 4 && accuracy !== null && accuracy <= 20 && cn0 !== null && cn0 >= 18) {
        quality = "נתוני Native מתאימים לניתוח משולב";
      } else if (view >= 10 && used <= 2) {
        quality = "Fix חלש — מומלץ שמיים פתוחים";
      }

      note.textContent = `${quality} · ${spoofStatus} · מדד הטעיה ${Math.round(spoofScore)}/100 · מדידה #${received} · ${new Date(finite(d.timestamp) || Date.now()).toLocaleTimeString("he-IL")}`;
    }

    previous = { lat, lon, time: locationTime, accuracy, speed };
  });
})();
